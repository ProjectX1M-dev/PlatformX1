...
export const useTradingStore = create<TradingState>((set, get) => ({
  ...
  getRobotSignalStats: async (robotId: string) => {
    try {
      ...
      // For simplicity, we'll count BUY signals as wins and SELL signals as losses
      // In a real system, you'd track the actual P&L of each trade
      const winCount = signalsData.filter(s => s.action === 'BUY').length;
      const lossCount = signalsData.filter(s => s.action === 'SELL').length;
      
      // Calculate win rate
      const winRate = totalTrades > 0 ? (winCount / totalTrades) * 100 : 0;
      
      // Calculate profit (simplified - in a real system you'd use actual P&L)
      // Here we'll use a simple model: each BUY signal adds $10, each SELL signal subtracts $5
      const profit = (winCount * 10) - (lossCount * 5);
